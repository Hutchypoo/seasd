...
class Main:
    ...
    def __init__(self):
        self.SetTitle('[GamerYT#3514 Twitch Token Gen And Checker]')
        self.clear()
...
Checking
    ...
    def SendWebhook(self,title,message,icon_url,thumbnail_url,proxy,useragent):
        try:
            ...
            message_to_send = {"embeds": [{"title": title,"description": message,"color": 65362,"author": {"name": "GamerYT#3514 DISCORD SERVER [CLICK HERE]","https://discord.gg/mhyEN6Hq8Z": "https://discord.gg/fWuVJuTc9D","icon_url": icon_url},"footer": {"text": "MADE BY The Gamer3514 ","icon_url": icon_url},"thumbnail": {"url": thumbnail_url},"timestamp": timestamp}]}
    ...
    def TitleUpdate(self):
        while True:
            self.SetTitle(f'[GamerYT#3514 Twitch Token Checker] ^| HITS: {self.hits} ^| BADS: {self.bads} ^| RETRIES: {self.retries} ^| WEBHOOK RETRIES: {self.webhook_retries} ^| THREADS: {active_count()-1}')
            sleep(0.1)
        ...
        try:
            ...
            if 'client_id' in response.text:
                ...
                if self.webhook_enable == 1:
                    self.SendWebhook('Twitch Token',token,'https://cdn.discordapp.com/attachments/854018635646763067/860196715822841917/Capture.PNG','https://cdn.discordapp.com/attachments/854018635646763067/860196715822841917/Capture.PNG',proxy,useragent)
            else:
                self.retries += 1
